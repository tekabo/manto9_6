...
public class GetUnOrderListThread extends Thread {
	...
	public GetUnOrderListThread(Context mContext, Handler mHandler, long userid,
			int pageIndex) {
		super();
	...
	public void run() {
		try {
			...
			OrderInfo orderInfo = rai.getUncompletedOrder(mContext,  userid, pageIndex);
			if (orderInfo != null) {
				Log.i("MyLog", "GetOrderListActivity---------------->"+orderInfo.toString());
				Log.i("MyLog", "测试滚动线程启动------");
				if (!running)
				{
					return;
				}
